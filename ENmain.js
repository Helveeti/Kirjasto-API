...
function success(pos) {
    ...
    naytaKartta(paikka);
    lisaaMarker(paikka, 'You are here', punainenIkoni);
    haeKirjastot(paikka);
}
...
function lisaaMarker(crd, teksti, osoite, ikoni, kirjasto) {
    L.marker([crd.latitude, crd.longitude], {icon: ikoni}).
    addTo(map).
    bindPopup(teksti + osoite).
    openPopup().
...
function haeKirjastot(crd) {
    const parameters = `geo=${crd.latitude},${crd.longitude}&distance=5&distanceunit=km&status=1`;
    fetch(api + parameters).then(function(answer) {
        ...
        for (let i = 0; i < kirjastot.length; i++) {
            const teksti = kirjastot[i].name.en + '<br>' + kirjastot[i].address.street.en + '<br><a href="' + kirjastot[i].homepage.en + '">Homepage</a>';
            const koordinaatit = {
                latitude: kirjastot[i].address.coordinates.lat,
                longitude: kirjastot[i].address.coordinates.lon,
            };
            lisaaMarker(koordinaatit, teksti, osoite, vihreaIkoni, kirjastot[i]);
        }
    });
}